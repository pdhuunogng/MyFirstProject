...
#include <iomanip>
/*
Write a program that stores the following hourly rates in an array name hourlyRates:  9.5, 6.4, 12.5, 5.5, 10.5. 
Your program should also create two arrays named workingHours and wages, each capable of storing five double-precision numbers. 
Using a for loop and a cin statement, have your program accept five user-input numbers into workingHours array when the program is run. 
Your program should store the product of the corresponding values in the hourlyRates and workingHours arrays in the wages array 
(for example, wages[1] = hourlyRates[1]*workingHours[1]) and display the output as a table consisting of three columns.
